...
describe("toBox", () => {
  ...
  test(
    ...
    async () => {
      ...
      const box = toBox(ref);
      await Promise.all([ref.update(firstValue), waitUntilBox(box)]);
      expect(box.getState().value).toEqual(firstValue);
    },
    TIMEOUT
  );
  test(
    ...
    async () => {
      ...
      const box = toBox(ref);
      if (box.getState().value !== null) {
        expect(box.getState().value).toEqual(STRING_VALUE);
      }
      await ref.set(STRING_VALUE);
  ...
  test(
    ...
    async () => {
      ...
      if (box.getState().value !== null) {
        expect(box.getState().value).toEqual({ a: "firstValue" });
        return;
      }
      await Promise.all([ref.update({ a: "firstValue" }), waitUntilBox(box)]);
      expect(box.getState().value).toEqual("FIRSTVALUE");
    },
    TIMEOUT
  );
